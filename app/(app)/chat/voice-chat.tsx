...
const VoiceChatScreen = () => {
    ...
    const startListening = async (language: string) => {
        ...
        try {
            ...
            Toast.show({ type: 'info', text1: 'Listening...' });
            startSilenceTimeout();
            animateButton(1.2);
            animateCircle(1.5, 1);
        } catch (error) {
        ...
        <View style={styles.container}>
            <Animated.View style={{ transform: [{ scale: buttonScale }] }}>
                <TouchableOpacity
                    ...
                    style={[styles.micButton, { backgroundColor: isListening ? 'red' : 'white' }]}>
                    <Image source={MicIcon} style={{ tintColor: isListening ? 'white' : 'black' }} />
                </TouchableOpacity>
            </Animated.View>

            <Animated.View style={[styles.circle, { transform: [{ scale: circleScale }], opacity: circleOpacity }]}>
