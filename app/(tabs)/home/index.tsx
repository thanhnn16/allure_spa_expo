  ...
  const renderServicesItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity marginR-15 marginB-5 style={[AppStyles.shadowItem, { borderRadius: 16, width: 230, height: 'auto' }]}>
        <Image 
          source={item.image_url ? { uri: item.image_url } : require('@/assets/images/home/service1.png')} 
          width={'100%'} 
          height={210} 
        ...
        <View paddingH-12 marginT-6>
          ...
          <Text style={{ color: '#8C8585' }}>{truncateText(item.description, 50)}</Text>
          <Text marginT-10 text70H style={{ color: '#A85A29' }}>{item.single_price.toLocaleString('vi-VN')} VNĐ</Text>
        </View>
      </TouchableOpacity>
    )
  }
